...
var HctSolver = class _HctSolver {
  ...
  static lerpPoint(source2, t, target) {
    return [
      source2[0] + (target[0] - source2[0]) * t,
      source2[1] + (target[1] - source2[1]) * t,
      source2[2] + (target[2] - source2[2]) * t
    ];
  }
  /**
  ...
  static setCoordinate(source2, coordinate, target, axis) {
    const t = _HctSolver.intercept(source2[axis], coordinate, target[axis]);
    return _HctSolver.lerpPoint(source2, t, target);
  }
  static isBounded(x) {
    return 0 <= x && x <= 100;
  }
...
var foundationYaml = await readFile(resolve(projectRoot, "config/foundation.yml"), "utf8");
var seedMatch = foundationYaml.match(/^\s*brand_seed_color:\s*["']?(#[0-9a-fA-F]{6})["']?\s*$/m);
if (!seedMatch) throw new Error("config/foundation.yml must define brand_seed_color as #RRGGBB");
var seed = seedMatch[1].toUpperCase();
var source = Hct.fromInt(argbFromHex(seed));
var roleNames = [
  "primary",
  "onPrimary",
  "primaryContainer",
...
var kebab = (name) => name.replace(/[A-Z]/g, (letter) => `-${letter.toLowerCase()}`);
var schemes = Object.fromEntries([false, true].map((dark) => {
...
var payload = `${JSON.stringify({ metadata, schemes }, null, 2)}
`;
var payloadSha256 = createHash("sha256").update(payload).digest("hex");
var cssBlocks = Object.entries(schemes).map(([name, roles]) => {
...
var systemDark = Object.entries(schemes.dark).map(([role, color]) => `    --md-sys-color-${role}: ${color};`).join("\n");
var css = `/* Generated by tools/material/generate_tokens.mjs. Do not edit. */
