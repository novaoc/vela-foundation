class Foundation::BillingController < ApplicationController
  ...
  def show
    ...
    @source = @organization.current_pricing_plan_source
  end

  def checkout
    return redirect_to(pricing_path, alert: "Self-serve upgrades are unavailable for manually assigned plans.") if
      @organization.manually_assigned_plan?
    return redirect_to(billing_path, alert: "Manage the existing subscription in the billing portal.") if
      @organization.billing_management_available?
  ...
  def portal
    unless @organization.billing_management_available?
