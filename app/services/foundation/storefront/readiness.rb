...
module Foundation
  module Storefront
    class Readiness
      ...
      def self.call(environment: ENV, settlement: false)
        ...
        if environment["VELA_HOLODEX_PREVIEW"] == "1"
          preview_mode = environment.fetch("STOREFRONT_PREVIEW_PAYMENT_MODE", "simulator")
          if preview_mode == "simulator" && !settlement_only
            errors = [ fulfillment_error ].compact
            return Result.new(ready?: errors.empty?, mode: "test simulator", errors: errors)
          end
          unless settlement_only || preview_mode == "stripe"
            return Result.new(ready?: false, mode: preview_mode, errors: [ "preview payment mode must be simulator or stripe" ])
          end
        end

        mode = environment.fetch("STOREFRONT_STRIPE_MODE",
          environment["VELA_HOLODEX_PREVIEW"] == "1" ? "test" : (Rails.env.production? ? "live" : "test"))
        ...
        errors << "preview Stripe checkout must use test mode" if environment["VELA_HOLODEX_PREVIEW"] == "1" && mode != "test"
        ...
        elsif environment["VELA_HOLODEX_PREVIEW"] == "1" && environment["STOREFRONT_PREVIEW_PAYMENT_MODE"] == "stripe" && environment["APP_HOST"].blank?
          errors << "preview Stripe checkout requires an explicit trusted APP_HOST"
        else
          begin
            StripeCheckoutSession.base_url(
              environment: environment,
              strict: Rails.env.production? || environment["VELA_HOLODEX_PREVIEW"] == "1" || mode == "live"
            )
